export const saveVersion = async (sourceCode, versionName, linkID, saveButtonState, versionBlocks, setVersionBlocks) => {
    try {
        ...
        const result = await fetch(`http://${import.meta.env.VITE_BACKEND_URL}/version/save-version`, requestOptions);
...
export const fetchVersionBlocks = async (id, setVersionBlocks) => {
    try {
        ...
        const result = await fetch(`http://${import.meta.env.VITE_BACKEND_URL}/version/fetch-version-blocks`, requestOptions);
...
export const fetchVersionBlockSourceCode = async (linkID, versionID, setSourceCode) => {
    try {
        ...
        const result = await fetch(`http://${import.meta.env.VITE_BACKEND_URL}/version/fetch-version-block-source-code`, requestOptions);
...
export const deleteVersionBlock = async (verID) => {
    try {
        ...
        const result = await fetch(`http://${import.meta.env.VITE_BACKEND_URL}/version/delete-version-block/${verID}`, { method: "DELETE" });
    }
    catch (error) {
        console.error(error);
    }
